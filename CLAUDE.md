...
| 「练习测试一体」「就用这个形式」 | 出成网页：练习/测试两个标签页，测试区三节（听辨+写形式+填句子），交卷自动计分回传。见下方「每日页的标准形态」 |
...
1. **听辨**（10-12 题）—— 听一个词/一句，二选一判断。靶子要选**听得出来**的差别，
   别学 2026-09-18 那次 `podía/podría` 出早了
2. **写形式**（5-6 题）—— 给提示写目标形式，测产出
3. **填进句子**（4 题）—— 放回真实句子里，测会不会用
